...
(async () => {
  ...
  await step("TNBS reskin: navy accent, light-only, chrome, zebra, search", async () => {
    const accent = await ev(() => getComputedStyle(document.documentElement).getPropertyValue("--accent").trim());
    if (accent !== "#1e3a6e") throw new Error("accent is not TNBS navy: " + accent);
    if (await page.$('[data-act="toggleTheme"]')) throw new Error("theme toggle still present (spec: light only)");
    if (!(await page.$(".appbar .brand .mark"))) throw new Error("navy appbar badge missing");
    const markBg = await page.$eval(".appbar .brand .mark", e => getComputedStyle(e).backgroundImage);
    if (!/data:image\/png/.test(markBg)) throw new Error("appbar mark is not the real badge artwork");
    const authMark = await ev(() => { const el = document.createElement("div"); el.className = "mark-badge-black"; document.body.appendChild(el); const bg = getComputedStyle(el).backgroundImage; el.remove(); return bg; });
    if (!/data:image\/png/.test(authMark)) throw new Error("black badge artwork missing");
    const barBg = await page.$eval(".appbar", e => getComputedStyle(e).backgroundColor);
    if (barBg !== "rgb(26, 53, 96)") throw new Error("appbar not navy: " + barBg);
    const zebra = await page.$eval("table.tbl tbody tr:nth-child(2)", e => getComputedStyle(e).backgroundColor);
    if (zebra !== "rgb(246, 249, 252)") throw new Error("zebra row not tinted: " + zebra);
    const thBg = await page.$eval("table.tbl thead th", e => getComputedStyle(e).backgroundColor);
    if (thBg !== "rgb(238, 242, 247)") throw new Error("th background wrong: " + thBg);
    // global search: type → popover with carrier + batch rows; navigate to a carrier
    await page.fill("#gs-in", "haul"); await wait(150);
    if (!(await page.$(".gs-pop .gs-row"))) throw new Error("global search returned no rows");
    await ev(() => { const r = document.querySelector('.gs-pop .gs-row[data-act="openCarrier"]'); if (r) ACT.openCarrier({ dot: r.getAttribute("data-dot") }); });
    await page.waitForSelector(".console-grid");
    await ev(() => ACT.nav({ to: "dashboard" })); await wait(120);
    await shot("02b-tnbs-chrome");
  });
  await step("new search: Columns layout (taller map + coverage, 3 columns)", async () => {
    ...
    const mh = await page.$eval("#wiz-map-holder", e => +e.getAttribute("data-h"));
    if (mh !== 430) throw new Error("map band should be 430 (owner: taller), got " + mh);
  ...
  await step("builder: name + lanes + facets + filters, live count", async () => {
    ...
    const zoneStyle = await page.$eval(".radius-fill", e => e.getAttribute("style") || "");
    if (!/--cat-1/.test(zoneStyle)) throw new Error("lane A zone not cat-colored: " + zoneStyle);
    const bandStyle = await page.$eval(".corridor-band", e => e.getAttribute("style") || "");
    if (!/--cat-2/.test(bandStyle)) throw new Error("lane B corridor not cat-colored: " + bandStyle);
    const legendHtml = await page.$eval(".map-legend", e => e.textContent);
    if (!/A ·/.test(legendHtml)) throw new Error("builder legend missing lane rows");
  ...
  await step("log outreach + sheet", async () => {
    ...
    const inset = await page.$eval("#wl-body tr.sel td:first-child", e => getComputedStyle(e).boxShadow);
    if (!/inset/.test(inset)) throw new Error("selected row missing 3px inset navy bar: " + inset);
  ...
  await step("users + guest gate (light only)", async () => {
    await ev(() => ACT.nav({ to: "users" })); await page.waitForSelector('[data-act="approveUser"]');
    await shot("11-users");
    await ev(() => ACT.logout()); await page.waitForSelector('[data-act="demoLogin"]');
